...
{
    public class DapperOracleController : Controller
    {
        // GET: DapperOracle
		...
		public IList<Payment> PaymentSearchFunc(DataTableAjaxPostModel model, out int filteredResultsCount, out long totalResultsCount)
		{
			...
			var result = new   AdoService<Payment>(conn, "PRI_LOC_PAYMENT").GetPaymentsFromDB(searchBy, take, skip, sortBy, sortDir, out filteredResultsCount, out totalResultsCount);
			if (result == null)
			{
				// empty collection...
